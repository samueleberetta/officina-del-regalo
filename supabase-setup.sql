...
-- 3. Tabella admin (credenziali login)
...
-- 8. Policy: chiunque può leggere i prodotti attivi
CREATE POLICY "Prodotti visibili a tutti" ON products
  FOR SELECT USING (true);

-- 9. Policy: solo service_role può modificare prodotti
CREATE POLICY "Solo admin modifica prodotti" ON products
  FOR ALL USING (auth.role() = 'service_role')
  WITH CHECK (auth.role() = 'service_role');

-- 10. Policy: solo service_role per ordini
