...
const VENDI_MESSAGE = `Ciao RetroStation! 👋
Vorrei vendervi la mia console.

• Modello:
• Condizioni:
• Accessori inclusi:
• Foto (allegherò a seguire)

Quanto mi potete offrire? Grazie!`;
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center">
            ...
            <button
              ...
              onClick={async () => {
                try {
                  await navigator.clipboard.writeText(VENDI_MESSAGE);
                  toast.success("Messaggio copiato! Incollalo nel DM.", {
                    duration: 4000,
                  });
                } catch {
                  // Fallback silenzioso se il clipboard non e' disponibile
                }
                window.open(
                  "https://ig.me/m/retrostation00s",
                  "_blank",
                  "noopener,noreferrer"
                );
              }}
              className="group inline-flex items-center gap-2 rounded-lg border border-neon-purple/60 text-neon-purple px-4 py-1.5 text-xs sm:px-5 sm:py-2 sm:text-sm font-heading tracking-wider hover:bg-neon-purple/10 hover:border-neon-purple transition-colors"
            >
              <svg
